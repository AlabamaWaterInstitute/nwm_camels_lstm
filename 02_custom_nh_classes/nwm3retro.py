"""
Adapted from the original code by the Neural Hydrology team (https://github.com/neuralhydrology)
Edited by Sonam Lama (GitHub: @slama0077)
"""
...
class NWM3Retro(BaseDataset):
    ...
    def __init__(self,
        ...
        super(NWM3Retro, self).__init__(cfg=cfg,
                                             is_train=is_train,
                                             period=period,
                                             basin=basin,
...
def load_attributes(data_dir: Path, basins: List[str] = None) -> pd.DataFrame:
    ...
    files = list(attributes_path.glob('*.parquet'))
    if not files:
    ...
    dfs = []
    for f in files:
        df = pd.read_parquet(f)  # make sure we read the basin id as str
